...
describe("renderer usability contract", () => {
  ...
  it("keeps the Letter App outreach workspace readable and scrollable", async () => {
    ...
    expect(stylesSource).toMatch(/\.letter-single-compose\s*\{[\s\S]*?grid-template-columns:\s*minmax\(420px,\s*0\.78fr\) minmax\(260px,\s*0\.22fr\)/);
    expect(stylesSource).toMatch(/\.letter-wide-panel\s*\{[\s\S]*?grid-column:\s*1 \/ -1/);
    ...
    expect(stylesSource).toMatch(/\.letter-form-field,[\s\S]*?font-weight:\s*700/);
