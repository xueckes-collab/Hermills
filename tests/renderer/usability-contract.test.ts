...
describe("renderer usability contract", () => {
  ...
  it("keeps dark desktop buttons readable in normal and disabled states", async () => {
    ...
    expect(stylesSource).toMatch(/\.hm-primary\s*\{[\s\S]*?background:\s*var\(--button-primary-bg\)/);
    expect(stylesSource).toMatch(/\.hm-secondary\s*\{[\s\S]*?background:\s*var\(--button-secondary-bg\)/);
    expect(stylesSource).toMatch(/\.hm-primary:disabled,[\s\S]*?\.hm-automation-banner button:disabled\s*\{[\s\S]*?opacity:\s*1/);
  });
  ...
  it("keeps outreach and drawers inside the desktop shell instead of creating page-level horizontal scroll", async () => {
    ...
    expect(stylesSource).toMatch(/\.hm-app-shell\s*\{[\s\S]*?max-width:\s*100%/);
    expect(stylesSource).toMatch(/\.hm-main\s*\{[\s\S]*?overflow-x:\s*hidden/);
    expect(stylesSource).toMatch(/\.hm-toolbar\s*\{[\s\S]*?flex-wrap:\s*wrap/);
    expect(stylesSource).toMatch(/\.hm-filter-row\s*\{[\s\S]*?flex-wrap:\s*wrap/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*920px\)[\s\S]*?\.hm-leads-layout,[\s\S]*?\.hm-campaign-review-grid\s*\{[\s\S]*?grid-template-columns:\s*1fr/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*920px\)[\s\S]*?\.hm-toolbar\s*\{[\s\S]*?display:\s*grid/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*920px\)[\s\S]*?\.hm-filter-row\s*\{[\s\S]*?grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(92px,\s*1fr\)\)/);
  ...
  it("keeps the Letter App outreach workspace readable and scrollable", async () => {
    ...
    expect(appSource).toContain("单封写信");
    expect(appSource).toContain("批量写信");
    expect(appSource).toContain("签名Logo");
    expect(appSource).toContain("公司资料");
    expect(appSource).toContain("导入批量客户");
    expect(appSource).toContain("hm-dashboard-actions");
    ...
    expect(appSource).toContain('className="hm-leads-draft-review"');
    expect(appSource).toContain('className="hm-single-result"');
    expect(appSource).toContain("api.outreachDrafts");
    expect(appSource).toContain("hm-batch-review-workspace");
    const composeView = sourceWindow(appSource, "{letterView === 'compose' ? (", 5200);
    expect(composeView).toContain('className="hm-single-compose"');
    expect(composeView).toContain("单个客户");
    expect(composeView).toContain("打开批量写信");
    expect(composeView).not.toContain("导入批量客户");
    expect(composeView).not.toContain("importLetterFile");
    const automationView = sourceWindow(appSource, "{letterView === 'automation' ? (", 6400);
    expect(automationView).toContain("导入批量客户");
    ...
    expect(automationView).toContain('className="hm-batch-send-card"');
    ...
    expect(mailView).toContain("你的发件邮箱");
    expect(mailView).toContain("邮箱授权码 / SMTP 密码");
    expect(mailView).toContain("获取 SMTP 授权码");
    expect(mailView).toContain("保存并测试邮箱");
    expect(mailView).toContain("高级设置（一般不用管）");
    ...
    expect(signatureView).not.toContain("自动追加邮件签名");
    expect(appSource).toContain("enabled: Boolean(text || hasLogo)");
    expect(appSource).toContain("html: ''");
    const quickLeadForm = sourceWindow(appSource, 'className="hm-form-grid quick-lead-form"', 1700);
    expect(quickLeadForm).toContain('OutreachField id="hm-quick-email"');
    expect(quickLeadForm).toContain('id="hm-quick-email"');
    ...
    expect(quickLeadForm).toContain('OutreachField id="hm-quick-website"');
    expect(quickLeadForm).toContain('id="hm-quick-website"');
    expect(quickLeadForm).toContain('type="url"');
    expect(quickLeadForm).toContain('autoComplete="url"');
    expect(quickLeadForm).toContain('onChange={(event) => setQuickWebsite(event.currentTarget.value)}');
    expect(stylesSource).toMatch(/\.hm-app-shell\s*\{[\s\S]*?overflow:\s*hidden/);
    expect(stylesSource).toMatch(/\.hm-main\s*\{[\s\S]*?overflow:\s*auto/);
    expect(stylesSource).toMatch(/\.hm-nav button,[\s\S]*?\.hm-primary,[\s\S]*?\.hm-secondary,[\s\S]*?min-height:\s*38px/);
    expect(stylesSource).toMatch(/\.hm-single-compose\s*\{[\s\S]*?grid-template-columns:\s*minmax\(420px,\s*0\.78fr\) minmax\(260px,\s*0\.22fr\)/);
    expect(stylesSource).toMatch(/\.hm-wide-panel\s*\{[\s\S]*?grid-column:\s*1 \/ -1/);
    expect(stylesSource).toMatch(/\.mail-simple-panel\s*\{[\s\S]*?max-width:\s*860px/);
    expect(stylesSource).toMatch(/\.mail-advanced-settings summary\s*\{[\s\S]*?高级设置|\.mail-advanced-settings summary\s*\{[\s\S]*?cursor:\s*pointer/);
    expect(stylesSource).toMatch(/\.signature-text-field textarea\s*\{[\s\S]*?min-height:\s*148px/);
    expect(stylesSource).toMatch(/\.hm-form-grid input,[\s\S]*?\.hm-import-textarea\s*\{[\s\S]*?color:\s*#111827/);
    expect(stylesSource).toMatch(/\.hm-form-field,[\s\S]*?font-weight:\s*700/);
    expect(stylesSource).toMatch(/\.hm-form-grid input,[\s\S]*?\.hm-import-textarea\s*\{[\s\S]*?pointer-events:\s*auto/);
    expect(stylesSource).toMatch(/\.hm-form-grid input,[\s\S]*?\.hm-import-textarea\s*\{[\s\S]*?-webkit-app-region:\s*no-drag/);
    expect(stylesSource).toMatch(/\.hm-thinking-panel\s*\{/);
    expect(stylesSource).toMatch(/\.hm-leads-draft-review\s*\{/);
    ...
    expect(stylesSource).toMatch(/@container \(max-width:\s*920px\)[\s\S]*?\.hm-sidebar\s*\{[\s\S]*?display:\s*grid/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*920px\)[\s\S]*?\.hm-brand\s*\{[\s\S]*?padding:\s*0/);
    expect(stylesSource).toMatch(/@media \(max-width:\s*1180px\)[\s\S]*?\.hm-leads-layout,[\s\S]*?\.hm-campaign-review-grid\s*\{[\s\S]*?grid-template-columns:\s*1fr/);
  });

  it("uses the SaaS blue outreach design foundation from the UI harness", async () => {
    const stylesSource = await readFile(projectFile("apps/renderer/src/styles.css"), "utf8");
    const letterShell = sourceWindow(stylesSource, ".hm-app-shell {\n  --button-primary-bg", 1800);
    const formControls = sourceWindow(stylesSource, ".hm-form-grid input:focus,", 1300);
    const primaryButton = sourceWindow(stylesSource, ".hm-primary", 900);
    const navigation = sourceWindow(stylesSource, ".hm-nav button", 1200);
    const stats = sourceWindow(stylesSource, ".hm-stat-card i.orange", 900);
    ...
    expect(navigation).toContain("color: var(--hm-text-secondary)");
    expect(navigation).toContain("background: var(--hm-bg-hover)");
    expect(navigation).toContain(".hm-nav button.active");
    expect(navigation).toContain("background: var(--hm-bg-active)");
    expect(navigation).toContain("color: var(--hm-blue)");
    ...
    expect(primaryButton).toContain("box-shadow: 0 8px 18px rgba(37, 99, 235, 0.16)");
    expect(stats).toContain(".hm-stat-card i.orange { color: #d97706; background: #fff7ed; }");
  });
  ...
  it("marks long-running outreach actions as visibly busy", async () => {
    ...
    expect(stylesSource).toMatch(/\.hm-primary\[aria-busy="true"\]::before,[\s\S]*?\.hm-secondary\[aria-busy="true"\]::before\s*\{[\s\S]*?animation:\s*hm-spin 0\.8s linear infinite/);
    expect(stylesSource).toMatch(/@keyframes hm-spin[\s\S]*?transform:\s*rotate\(360deg\)/);
  });
  ...
  it("keeps outreach SMTP failures diagnosable from the visible error banner", async () => {
    ...
    expect(stylesSource).toMatch(/\.hm-alert-copy\s*\{[\s\S]*?color:\s*#881337/);
    expect(stylesSource).toMatch(/\.hm-detail-panel\s*\{[\s\S]*?max-height:\s*calc\(100dvh - 136px\)/);
    expect(stylesSource).toMatch(/\.hm-detail-panel\s*\{[\s\S]*?overflow:\s*auto/);
    expect(stylesSource).toMatch(/\.hm-sticky-actions\s*\{[\s\S]*?position:\s*sticky/);
    expect(stylesSource).toMatch(/\.hm-sticky-actions\s*\{[\s\S]*?box-shadow:\s*0 -10px 22px/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*540px\)[\s\S]*?\.hm-action-row\s*\{[\s\S]*?display:\s*grid/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*540px\)[\s\S]*?\.hm-action-row \.hm-primary,[\s\S]*?\.hm-action-row \.hm-secondary\s*\{[\s\S]*?white-space:\s*normal/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*540px\)[\s\S]*?\.hm-action-row \.hm-primary,[\s\S]*?\.hm-action-row \.hm-secondary\s*\{[\s\S]*?overflow-wrap:\s*anywhere/);
    expect(stylesSource).toMatch(/@container \(max-width:\s*540px\)[\s\S]*?\.hm-alert-copy\s*\{[\s\S]*?white-space:\s*normal/);
