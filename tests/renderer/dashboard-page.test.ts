...
describe("dashboard page redesign contract", () => {
  it("uses the outreach design system for today's workspace", async () => {
    ...
    expect(dashboardView).not.toContain("letter-action-card");
  });
  ...
  it("styles the dashboard as a focused action hub", async () => {
    ...
    expect(stylesSource).toMatch(/\.letter-dashboard-workspace\s*\{[\s\S]*?display:\s*grid/);
    expect(stylesSource).toMatch(/\.letter-dashboard-stats\s*\{[\s\S]*?grid-template-columns:\s*repeat\(5/);
    expect(stylesSource).toMatch(/\.letter-dashboard-actions\s*\{[\s\S]*?grid-template-columns:\s*repeat\(4/);
    expect(stylesSource).toMatch(/\.letter-dashboard-card-button\s*\{[\s\S]*?min-height:\s*140px/);
  });
});
